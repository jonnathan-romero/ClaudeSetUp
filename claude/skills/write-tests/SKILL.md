---
name: write-tests
description: Write pytest tests for one specified Python file — either new code you just wrote or legacy code you don't yet trust. ALWAYS trigger when the user says "write tests for this file", "add tests for <file.py>", "this module has no tests", "cover this with tests", "write unit tests", "add pytest tests", "test this function", "I need tests before I refactor this", or names a .py file and asks for tests. Branches on two modes that need OPPOSITE discipline — new code (tests encode intended behavior, a failure means the code is wrong) and legacy (characterization tests encode ACTUAL behavior including bugs, a failure means your expectation was wrong). Prefers output-based assertions over state-based over mock-call verification, and proves the suite actually fails when the code breaks using the shared sabotage check rather than trusting coverage. Do NOT use to audit an EXISTING suite across a repo for tests that cannot fail (that is the `@test-suite-auditor` agent), to configure pytest/conftest/CI/coverage gates, to debug one specific failing or flaky test, or for general code review and bug-hunting (use /code-review). "Clean up / tidy / restyle these tests" when they already exist and already pass is /simplify or code-simplifier, not this skill — this skill triggers when tests are MISSING or a module needs new coverage; reach for it on "clean up these tests" only if the cleanup is actually "these tests do not check anything, rewrite them to assert real behavior". Python and pytest only.
