#!/usr/bin/env bash
# agent-brief: open an editable side-by-side diff of a brief file in VS Code
...
#     <dest>      final path to write (e.g. .briefs/01-data-layer.md)
...
if command -v code >/dev/null 2>&1; then
    ...
    cleanup_left=""
    if [ ! -f "$DEST" ]; then
        left="$(mktemp -t agent-brief-empty-XXXX.md)"
        cleanup_left="$left"
    fi
