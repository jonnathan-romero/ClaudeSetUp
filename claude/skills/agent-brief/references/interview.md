...
7. **The dials.** Operating Mode — Converge (default for a from-step brief, or **Escalate-on-Stuck**, which is Converge plus an ask-on-blocker escape valve), Continuous (standalone only), or Supervised (where are the checkpoints)? Per locked decision: Locked / Split / Open? And the **Execution route** (a *binding* the executor follows): Two subagents (small, inline), Workflow (when you want a real build→review retry loop), or Fresh session (a large brief wanting a clean context) — recommend one from the mode (Continuous/Escalate-on-Stuck ⇒ Workflow; small Converge ⇒ Two subagents; large ⇒ Fresh session), ask with AskUserQuestion. → **Operating Mode, Route + Mutability**.
