"""Deterministic regex audit for the mechanically-detectable prompt anti-patterns + folklore tactics.

Covers the regex-tractable subset: 7 of the 15 anti-patterns in references/anti-patterns.md
(AP-2, 3, 5, 7, 9, 11, 15) plus an all-caps style check, and 5 folklore tactics. The remaining
anti-patterns (contradicting examples, conflicting persona, false-premise hallucination, etc.)
need manual review — see the improve-mode pipeline in SKILL.md.
...
def detect_negation_only(text: str) -> list[Issue]:
    ...
    issues: list[Issue] = []
    pattern = re.compile(
        r"\b(don't|do not|never|avoid|don't ever|stop)\s+\w+", re.IGNORECASE
    )
...
def detect_instruction_overload(
    text: str, threshold: int = 8
...
def detect_format_ambiguity(text: str) -> list[Issue]:
    ...
    issues: list[Issue] = []
    pattern = re.compile(
        r"\b(output|return|respond)\s+(?:as |with |in )?(json|a list|a table)\b",
        re.IGNORECASE,
    )
    for m in pattern.finditer(text):
        ...
        has_schema = any(
            kw in window for kw in ["{", "schema", "field", "key:", '"name"', '"type"']
        )
        if has_schema:
            continue
        issues.append(
...
def detect_over_constraining(text: str) -> tuple[list[Issue], int]:
    ...
    pattern = re.compile(
        r"\b(never|must not|do not|don't ever|don't ever|forbidden|prohibited)\b",
        re.IGNORECASE,
    )
...
def detect_cot_on_reasoning(text: str, target_model_class: str) -> list[Issue]:
    ...
    if target_model_class != "reasoning":
        return []
    pattern = re.compile(
        r"\b(think step by step|walk through your reasoning|think carefully|first do|then do)\b",
        re.IGNORECASE,
    )
    issues: list[Issue] = []
    for m in pattern.finditer(text):
...
def detect_all_caps_shouting(text: str) -> list[Issue]:
    ...
    pattern = re.compile(
        r"\b(CRITICAL|IMPORTANT|MUST|NEVER|MANDATORY|REQUIRED)\b(?!\w)"
    )
    issues: list[Issue] = []
...
FOLKLORE_PATTERNS: list[tuple[str, re.Pattern[str], str, str]] = [
    (
        "FOLK-tipping",
        re.compile(
            r"\b(?:tip|reward|pay) (?:you|me)?\s*\$?\d+|\$\d{2,}\s*tip|i('ll| will) pay you",
            re.IGNORECASE,
        ),
    ...
    (
        "FOLK-threat",
        re.compile(
            r"\byour job depends|i('ll| will) shut you off|you'?ll be (killed|deleted|punished)|punish(ed)? if|fired if",
            re.IGNORECASE,
        ),
    ...
    (
        "FOLK-expert-persona",
        re.compile(
            r"\b(world[- ]class|nobel laureate|stanford|harvard|mit|10\+? years|20\+? years|30\+? years|senior expert|world's best|leading expert)\b",
            re.IGNORECASE,
        ),
    ...
    (
        "FOLK-emotion-prompt",
        re.compile(
            r"\bvery important to (?:my career|me|my family|me personally)|this is critical for me|my (?:job|life) depends\b",
            re.IGNORECASE,
        ),
