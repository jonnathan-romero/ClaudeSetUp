...
Plan at two resolutions: **detail the near term, keep the far term coarse**, and make decisions concrete only as the work reaches them (the last responsible moment). The plan is a stripped-down spine with suggestions, not a fully-specified blueprint — over-specifying early imputes the first session's guesses onto work it cannot yet understand.

This skill is **human-in-the-loop**. It advises, sizes, and proposes; the user decides. It is not for producing a fully-specified, decide-everything-up-front brief for an autonomous agent to execute alone — that is a different kind of plan.
...
|------|------------|----------|-----------|
| **master-plan** | the whole multi-plan effort | `.plan/master-plan.md` | one north-star goal; **never contains steps** |
...
```
.plan/
  interview.md          # durable record of the planning interview (the full "why")
  master-plan.md        # only when >1 plan exists or is clearly coming
...
- **Handoffs and research are NOT stored in `.plan/`.** They reuse existing conventions: handoffs → `.handoffs/` (owned by the `handoff` skill), research → `.research/`. Plan files link to them by relative path (e.g. `../.research/02-auth-options.md`).
...
4. **Save the interview findings to `.plan/interview.md`** — the full shared understanding, including the deferred and far-term material that won't go in the plan yet (use the interview reference). Skip if no interview was run (the user gave a ready spec).
5. Write the plan file(s) from the templates in [`references/templates.md`](references/templates.md), through the diff review (below) — capturing only what the near-term needs, drawing the rest from `interview.md` later.

### next — work the next step

1. Identify the in-progress / next step in the active plan.
2. Do the work (build or research).
...
4. For a **research** step, the conclusion lives in the step's `Outcome` line. Also write the supporting detail to `.research/NN-*.md` and link it from `Outcome` — **skip the file only when the finding is trivial** (a sentence or two of conclusion with no reasoning, data, or code worth re-reading). Capture the conclusion in the plan **before** moving on; an unwritten finding evaporates at the next reset. If the finding is a cross-cutting choice, also record it in `Decisions Made`.
...
- **PROMOTE** — lift the step into its own new child plan. Number it **one greater than the highest existing `NN` in `.plan/`** (`ls .plan/` first — numbers are unique IDs, not execution order, so the new plan may land out of arc; record the real order in the master Plans table). Mark the old step resolved with a pointer — `- [x] Step 4 → promoted to Plan NN` (counts as resolved, not pending, for status) — and **create `master-plan.md` if it doesn't yet exist** (you now provably have >1 plan), adding a row to its Plans table.
...
- **Part of a master plan:** read `master-plan.md`'s Plans table first, then orient the active plan **within the whole arc** — not just in isolation:
...
- **Diff-review these (substantive):** creating a child plan, master plan, or `interview.md`; split/promote rewrites; writing or changing a Decision; rewriting or re-planning existing step text.
