...
4. For a **research** step, the conclusion lives in the step's `Outcome` line. Also write the supporting detail to `.research/NN-*.md` and link it from `Outcome` — **never skip the file writing**. Capture the conclusion in the plan **before** moving on; an unwritten finding evaporates at the next reset. If the finding is a cross-cutting choice, also record it in `Decisions Made`.
