...
description: Repo-wide health audit that orchestrates parallel auditor agents across three co-equal families - duplication, simplification, and documentation drift. ALWAYS trigger when the user wants to audit a whole repository rather than review a diff - "find repeated code", "what could be generalized", "DRY audit", "what should we simplify", "find overly complex code", "find dead code", "what can we delete", "do the docs still match the code", "are the docs accurate", "find documentation drift", "where is the tech debt", "code health audit", "audit the repo for code quality/health", "what needs refactoring" - or before a refactor, a cleanup sprint, or a release. Maps the repo with codebase-explorer scouts, runs the mechanical detectors once (jscpd for clones, lizard plus per-ecosystem linters for complexity and dead code) and inventories the doc surface, applies each family's own evidence gate, fans out duplication-auditor, simplification-auditor, and docs-drift-auditor agents, then merges everything into one ranked report. Read-only - proposes changes, never performs them. Do NOT use to review or simplify the current diff or recently-changed files (use /simplify or code-simplifier, which edit in place), or to hunt bugs (use /code-review). For a small repo or one narrow question, invoke @duplication-auditor, @simplification-auditor, or @docs-drift-auditor directly - this skill is the multi-agent orchestrator and it is deliberate overhead.
