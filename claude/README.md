...
| **realign-plan** | The periodic reconciliation pass over a whole `.plan/` effort: sweeps every plan + `.research/` + the actual code for cross-plan contradictions, assumptions that hardened into decisions, and plan-vs-code drift, then interviews to re-confirm intent and compacts accreted history — rewriting the plan files in place. Pairs with `rolling-plan`; human-in-the-loop. | scripts, references |
...
| **code-health-audit** | Repo-wide code health audit, orchestrated across two families. Spawns `codebase-explorer` scout(s) to map the repo (shared-code layer, conventions, exclusion trees) while running the detectors **once** globally (jscpd for clones, lizard + per-ecosystem linters for complexity/dead code), gates candidates on git change history, then fans out `@duplication-auditor` and `@simplification-auditor` agents and merges them into one ranked report. Never shards by directory; resolves cross-family conflicts toward leaving code duplicated; collapses to a single agent on small repos. | scripts · `/code-health-audit [path]` |
| **comment-cleanup** | Cleans up the comments and docstrings in one specified file: reads it plus a hop or two of surrounding code, drops redundant/obvious comments, and re-adds only minimal succinct ones — while preserving functional directives (shebangs, `# noqa`/`# type` pragmas, license headers, runtime docstrings) and changing comments only, never code. | `/comment-cleanup <file>` |
