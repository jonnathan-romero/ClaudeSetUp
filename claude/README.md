...
| **test-suite-auditor** | Repo-wide audit of an existing Python/pytest suite for the property nobody measures: **does it fail when the code is wrong?** Built on the finding that coverage answers a different question misleadingly — 70% of real high-priority bugs at Google were coupled to a mutant on lines the tests *already covered*. **A lint hit is not a finding**: ruff `PT` fires 1,296 times across eight elite repos and ~90% of that is style, so a finding must name the specific way the suite stays green while the code is wrong, and carry a **sabotage result** proving it wherever that is cheap. Runs the shared AST census, ruff twice (the *delta* is the team's sanctioned exemption set and is never re-litigated), `pytest -ra` for the skip/XPASS census, `-n 4 --dist load` for order dependence — which is deterministic where a seed sweep is luck (seed 42 was clean on a planted defect) — and coverage *contexts* joined against the assertion-free list to find lines covered but never checked. **Never reports a mutation score or a coverage percentage as a verdict**: Google abandoned the absolute score as unactionable, and both metrics stop being reliable exactly when the code under test may already be buggy, which is always this agent's situation. Read-only in the sense that matters — never edits a test or tracked source, never commits — but it *does* execute the suite and briefly sabotages a source file inside a checksum-verified restore, so it refuses to run on a dirty tree and says so in the report rather than claiming a purity it lacks. Carries a large "when NOT to report" list (REVIEW-bucket assertion-free tests, declared `xdist_group` coupling, repo-chosen private access, dict order — which *is* guaranteed). | Read, Grep, Glob, Bash, Write |
...
|------|-----------------|--------------|
| **ruff-format.sh** | PostToolUse (`Write\|Edit\|MultiEdit\|NotebookEdit`) | Formats edited `.py`/`.pyi`/`.ipynb` files with `ruff format` (Black-compatible; formats notebooks natively) — prefers a `ruff` on `PATH`, falls back to `uvx ruff`. Extracts `notebook_path` too, so `NotebookEdit` edits get formatted. |
...
| **write-tests** | Writes pytest tests for one specified Python file. **Declares its mode first**, because the two jobs invert every failure: in *new-code* mode tests encode intended behavior and a failure means the code is wrong; in *legacy* mode they are characterization tests encoding **actual** behavior including bugs, and a failure means the expectation was wrong — with expected values obtained by asserting a deliberate placeholder, running it, and pinning what the failure reports, never by guessing. Reads the **spec, not the implementation**, since deriving assertions from the code under test yields an oracle that can only confirm what the code already does (the measured LLM failure: models "capture the actual program behaviour rather than the expected one," and buggy input steers them into validating the bug). Assertion spine is a ladder — output-based, then state-based, then mock-call verification as a **last resort**, where needing the last rung is a signal about the *design*. Doubles are gated on two questions (unmanaged dependency? outgoing command?) plus two absolutes: never mock what you don't own, always `spec=`/`create_autospec`. Stop condition is **not** coverage but the shared sabotage check — break 3–5 load-bearing statements and prove the suite goes red — with a real mutmut run as an explicit opt-in escalation, carrying the operational facts that otherwise cost a run (`--paths-to-mutate` no longer exists; never `mutmut apply`; a timeout is an invalid run, not a survivor). | `/write-tests <file>` · shared `sabotage_check`, `test_census`, `test_failure_modes` |
...
| **jupyter-notebooks** | Creates, edits, executes, and converts `.ipynb` notebooks. Notebooks are assembled by `scripts/nbtool.py` from a cheap `# %%` percent-format draft — never hand-written JSON, because a non-canonical file (`indent=1`, sorted keys) is rewritten wholesale by Jupyter's first save and the next diff is the entire notebook. `nbtool` reuses the output file's existing cell ids on rebuild, which is what bare `jupytext --to ipynb` cannot do (it regenerates random ids every conversion), and validates before every write because **`nbformat.write()` logs an invalid notebook and saves it anyway**. Metadata — papermill `parameters` tags, nbconvert hide tags, reveal.js slide types — goes through `nbtool meta`, since `NotebookEdit` structurally has no metadata parameter; `NotebookEdit` handles cell *source* only. Execution branches on **project vs standalone**, the distinction that breaks notebooks most often: bare `uv run --with` builds an *isolated* env where the user's own package is missing, while `uv run --project <dir> --with nbclient` *layers* onto the project env; `--with ipykernel` is needed only in the isolated case, and `jupyter execute` writes **nothing** without `--inplace` (which makes the bare form the correct CI gate). Delivery is a decision rule — report → execute and keep outputs; working notebook → execute to prove it runs, then strip; no env → deliver unexecuted and say so — and failures split by cause, retrying fixable code bugs but stopping dead on absent data or credentials. Export probes rather than assumes: `--to pdf` calls pandoc for every markdown cell and dies `PandocMissing` without it, so the skill tests for pandoc + TeX and otherwise uses webpdf under `env -u DISPLAY` (with `DISPLAY` set it hangs at 100% CPU). Pins `nbconvert>=7.17.1` — two path-traversal CVEs affect 6.5 onward. Asks before `uv add`, never configures nbstripout, never installs system packages. | scripts, references |
