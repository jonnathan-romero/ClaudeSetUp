...
| **code-health-audit** | Repo-wide code health audit, orchestrated across two families. Runs the detectors **once** globally (jscpd for clones, lizard + per-ecosystem linters for complexity/dead code), gates candidates on git change history, then fans out `@duplication-auditor` and `@simplification-auditor` agents and merges them into one ranked report. Never shards by directory; resolves cross-family conflicts toward leaving code duplicated; collapses to a single agent on small repos. | scripts · `/code-health-audit [path]` |
